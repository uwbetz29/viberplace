import { auth } from "@/lib/auth";
import { getDb } from "@/lib/db";
import { redirect } from "next/navigation";
import { AdminConsole } from "@/components/admin-console";

export default async function AdminPage() {
  const session = await auth();
  if (!session?.user) redirect("/");

  const sql = getDb();
  const username = (session.user as any).username as string;

  const users = await sql`SELECT id, role FROM users WHERE username = ${username}`;
  if (users.length === 0 || users[0].role !== "admin") redirect("/dashboard");

  // Fetch all data for admin
  const allUsers = await sql`
    SELECT u.*,
      (SELECT COUNT(*) FROM apps WHERE user_id = u.id) as app_count,
      (SELECT COUNT(*) FROM apps WHERE user_id = u.id AND status = 'published') as published_count
    FROM users u
    ORDER BY u.created_at DESC
  `;

  const allApps = await sql`
    SELECT a.*, u.username, u.avatar_url
    FROM apps a
    JOIN users u ON a.user_id = u.id
    ORDER BY a.created_at DESC
  `;

  const stats = {
    totalUsers: allUsers.length,
    totalApps: allApps.length,
    publishedApps: allApps.filter((a) => a.status === "published").length,
    unlistedApps: allApps.filter((a) => a.status === "unlisted").length,
  };

  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          <svg className="w-6 h-6 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" />
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          <h1 className="text-3xl font-bold">Admin Console</h1>
        </div>
        <p className="text-foreground/50 mt-1">Manage users and apps across Viberplace</p>
      </div>

      <AdminConsole
        users={allUsers.map((u) => ({
          id: u.id as number,
          username: u.username as string,
          name: u.name as string,
          email: u.email as string,
          avatar_url: u.avatar_url as string | null,
          role: (u.role as string) || "member",
          provider: u.provider as string,
          app_count: Number(u.app_count),
          published_count: Number(u.published_count),
          created_at: u.created_at as string,
        }))}
        apps={allApps.map((a) => ({
          id: a.id as number,
          name: a.name as string,
          slug: a.slug as string,
          tagline: a.tagline as string,
          url: a.url as string,
          status: a.status as string,
          username: a.username as string,
          avatar_url: a.avatar_url as string | null,
          created_at: a.created_at as string,
        }))}
        stats={stats}
      />
    </div>
  );
}
